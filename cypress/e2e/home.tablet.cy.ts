describe("Home Page - Pokémon List (Tablet)", () => {
  beforeEach(() => {
    // Define viewport para tablet
  ...
  describe("Interações com Cards de Pokémon", () => {
    it("não deve mostrar botão de expandir em tablets", () => {
      // Aguarda os pokémons carregarem
      cy.wait(3000);

      // Verifica se o botão de expandir não está presente em tablets
      cy.get('[data-testid="pokemon-card"]')
        .first()
        .within(() => {
  ...
  describe("Layout específico para tablet", () => {
    it("deve exibir layout de duas colunas", () => {
      // Aguarda os pokémons carregarem
      cy.wait(3000);

      // Verifica se o contêiner tem a classe grid-cols-2 para tablet
